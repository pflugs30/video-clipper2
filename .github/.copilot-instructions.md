...
- **Module System**: **ESM (ES Modules) ONLY** - No CommonJS allowed
...
**Important**: This project uses `"type": "module"` in package.json, meaning all JavaScript files are treated as ES modules. Both the renderer process and main Electron process use ESM exclusively.
...
### Import Style
**This project uses ESM (ES Modules) exclusively - no CommonJS!**
...
// const React = require('react');  // DON'T DO THIS
// module.exports = Component;      // DON'T DO THIS
```

**For Electron main process files (Node.js environment):**
- Use `import.meta.url` instead of `__dirname`
- Use ES module imports for all dependencies
- Example pattern:
```typescript
import { fileURLToPath } from 'url';
import path from 'path';
...
- **CommonJS syntax** - `require()`, `module.exports`, `exports` - **NEVER use these! This project is ESM-only**
- `__dirname` or `__filename` directly in Electron main process (use `import.meta.url` pattern shown above)
